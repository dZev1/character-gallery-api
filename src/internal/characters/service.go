...
func (s *Service) Create(ctx context.Context, character *Character) (*Character, error) {
	...
	_ = s.cache.Set(ctx, fmt.Sprintf("%d", character.ID), character, 0)

	return character, nil
}

func (s *Service) ReadByID(ctx context.Context, id CharacterID) (*Character, error) {
	character := &Character{}
	...
	_ = s.cache.Set(ctx, fmt.Sprintf("%d", id), character, 0)
	return character, nil
}

func (s *Service) ReadAll(ctx context.Context, page int) ([]Character, uint64, error) {
...
func (s *Service) Update(ctx context.Context, character *Character) (*Character, error) {
	...
	_ = s.cache.Delete(ctx, fmt.Sprintf("%d", character.ID))
